...
# Add asyncio event loop initialization for compatibility with async libraries
import asyncio
try:
...
# ensure a stable IBKR client id across Streamlit reruns
if "ibkr_client_id" not in st.session_state:
    env_id = os.getenv("IBKR_CLIENT_ID")
    if env_id and env_id.isdigit():
        st.session_state["ibkr_client_id"] = int(env_id)
    else:
        st.session_state["ibkr_client_id"] = 12345  # static id to use for testing
            ...
            if broker_instance:
                if selected_broker == 'Alpaca':
                    ...
                    if hasattr(broker_instance, 'ib') and broker_instance.ib.isConnected():
                        ...
                        st.warning(f"⚠️ {selected_broker} Not Connected")
                        st.caption("Make sure TWS/IB Gateway is running")
            else:
                st.error(f"❌ {selected_broker} Unavailable")
        except Exception as e:
            st.error(f"❌ {selected_broker} Error: {str(e)[:50]}")
...
with tab2:
    ...
    selected_broker_trade = st.session_state.get('selected_broker', 'Alpaca')
    
    try:
        broker_instance = get_broker_instance(selected_broker_trade)
        if broker_instance:
            if selected_broker_trade == 'Alpaca':
                try:
                    account = broker_instance.api.get_account()
                    broker_connected = True
                except:
                    broker_connected = False
            elif selected_broker_trade == 'IBKR':
                if hasattr(broker_instance, 'ib') and broker_instance.ib.isConnected():
                    broker_connected = True
    except Exception as e:
        broker_connected = False
    ...
    if not broker_connected:
        st.warning(f"⚠️ **{selected_broker_trade} Not Connected** - Connect broker in Broker Selection to execute trades")
    else:
        st.success(f"✅ **{selected_broker_trade} Connected** - Ready to execute trades")
                    ...
                    if INTEGRATIONS_AVAILABLE:
                        try:
                            ...
                            if current_price is None:
                                hist = yf.Ticker(selected_symbol).history(period='1d')
                                if not hist.empty:
                                    price_val = hist['Close'].iloc[-1]
                                    if pd.notna(price_val) and not np.isnan(price_val):
                                        current_price = float(price_val)
...
if tab_deployment:
    with tab_deployment:
        ...
        # Check broker connection (using selected broker) - cache in session state
        selected_broker_deploy = st.session_state.get('selected_broker', 'Alpaca')
        broker_cache_key = f'deploy_broker_connected_{selected_broker_deploy}'
        
        # Initialize or retrieve cached connection status
        if broker_cache_key not in st.session_state:
            st.session_state[broker_cache_key] = False
            st.session_state[f'deploy_broker_instance_{selected_broker_deploy}'] = None
            st.session_state[f'deploy_buying_power_{selected_broker_deploy}'] = 0.0
            st.session_state[f'deploy_account_status_{selected_broker_deploy}'] = None
        
        broker_connected = st.session_state[broker_cache_key]
        broker_instance = st.session_state[f'deploy_broker_instance_{selected_broker_deploy}']
        buying_power = st.session_state[f'deploy_buying_power_{selected_broker_deploy}']
        account_status = st.session_state[f'deploy_account_status_{selected_broker_deploy}']
        
        # Only check connection if not already connected or if explicitly reconnecting
        if not broker_connected or broker_instance is None:
            with st.spinner(f"🔌 Connecting to {selected_broker_deploy}..."):
                try:
                    print(f"🔌 Attempting to connect to {selected_broker_deploy}...")
                    ...
                    if broker_instance:
                        if selected_broker_deploy == 'Alpaca':
                            ...
                            print(f"✅ Alpaca connected: status={account_status}, buying_power=${buying_power:,.2f}")
                            ...
                            if hasattr(broker_instance, 'ib') and broker_instance.ib.isConnected():
                                ...
                                buying_power = account_info.get('buying_power', 0.0) if account_info else 0.0
                                print(f"✅ IBKR connected: status={account_status}, buying_power=${buying_power:,.2f}")
                            else:
                                broker_connected = False
                                print(f"❌ IBKR not connected or no IB instance")
                        
                        # Cache the connection status
                        st.session_state[broker_cache_key] = broker_connected
                        st.session_state[f'deploy_broker_instance_{selected_broker_deploy}'] = broker_instance
                        st.session_state[f'deploy_buying_power_{selected_broker_deploy}'] = buying_power
                        st.session_state[f'deploy_account_status_{selected_broker_deploy}'] = account_status
                    else:
                        broker_connected = False
                        print(f"❌ No broker instance returned for {selected_broker_deploy}")
                except Exception as e:
                    broker_connected = False
                    st.session_state[broker_cache_key] = False
                    broker_name_display = selected_broker_deploy
                    error_msg = str(e)
                    print(f"❌ Broker connection error: {error_msg}")
                    if selected_broker_deploy == 'Alpaca':
                        st.warning(f"⚠️ **{broker_name_display} Connection**: {error_msg}")
        ...
        # Display connection status with reconnect and refresh buttons
        col_status, col_refresh, col_reconnect = st.columns([4, 0.8, 0.8])
        with col_status:
            if broker_connected:
                broker_name_display = selected_broker_deploy
                st.success(f"✅ **{broker_name_display} Connected** - Account Status: {account_status.upper()} | Buying Power: ${buying_power:,.2f}")
            else:
                st.error(f"❌ **{selected_broker_deploy} Not Connected** - Please check your broker connection")
        
        with col_refresh:
            if broker_connected and st.button("🔄 Refresh", key="refresh_buying_power"):
                # Refresh buying power without full reconnection
                try:
                    if selected_broker_deploy == 'Alpaca' and broker_instance:
                        account = broker_instance.api.get_account()
                        buying_power = float(account.buying_power)
                        st.session_state[f'deploy_buying_power_{selected_broker_deploy}'] = buying_power
                        st.success(f"Updated: ${buying_power:,.2f}")
                    elif selected_broker_deploy == 'IBKR' and broker_instance:
                        account_info = broker_instance.get_account_info()
        ...
        with col_reconnect:
            if st.button("🔌 Reconnect", key="reconnect_deploy_broker"):
                # Clear cached connection status to force reconnection
                st.session_state[broker_cache_key] = False
                st.session_state[f'deploy_broker_instance_{selected_broker_deploy}'] = None
                print(f"🔄 Reconnecting to {selected_broker_deploy}...")
                st.rerun()
        
        # Debug info - show AFTER connection attempt
        with st.expander("🔍 Connection Debug Info", expanded=False):
            st.write(f"**Broker:** {selected_broker_deploy}")
            st.write(f"**Cache Key:** {broker_cache_key}")
            st.write(f"**Cached Connected:** {st.session_state.get(broker_cache_key, 'NOT SET')}")
            st.write(f"**Broker Instance:** {st.session_state.get(f'deploy_broker_instance_{selected_broker_deploy}') is not None}")
            st.write(f"**Current Connected:** {broker_connected}")
            st.write(f"**Buying Power:** ${st.session_state.get(f'deploy_buying_power_{selected_broker_deploy}', 0.0):,.2f}")
            st.write(f"**Account Status:** {st.session_state.get(f'deploy_account_status_{selected_broker_deploy}', 'None')}")
        ...
        with col_deploy2:
            ...
            if ticker_deploy:
                ...
                # For IBKR, skip other broker methods and go straight to yfinance (more reliable for Paper Trading)
                # For Alpaca, try additional broker methods before falling back to yfinance
                if current_price is None and selected_broker_deploy == 'Alpaca' and broker_connected and broker_instance:
                    # Method 2: Try Alpaca-specific price fetching
                    try:
                    ...
                    # Method 3: Try Alpaca get_latest_trade
                    if current_price is None:
                        try:
                ...
                if current_price is None:
                    try:
                        ...
                        st.info("💡 Try a different ticker symbol or check your internet connection")
                        current_price = None
            ...
            # Store current price in session state for use in PREPARE button
            if current_price:
                st.session_state['deploy_current_price'] = current_price
            else:
                st.session_state['deploy_current_price'] = None
        ...
        if broker_connected and broker_instance:
            ...
            st.info(f"💡 Connect to {selected_broker_deploy} to see current positions")
        ...
        if 'prepared_action' not in st.session_state:
            st.session_state.prepared_action = "BUY"
        ...
        with col_prepare:
            ...
            if prepare_disabled:
                reasons = []
                if not ticker_deploy:
                    reasons.append("No ticker entered")
                if deploy_current_price is None:
                    reasons.append("No price available")
                st.caption(f"⚠️ PREPARE disabled: {', '.join(reasons)}")
            
            if st.button("🔍 PREPARE", type="primary", use_container_width=True,
                        disabled=prepare_disabled):
                st.write(f"🔍 PREPARE button clicked! ticker={ticker_deploy}, price={deploy_current_price}")
                if ticker_deploy and deploy_current_price is not None and deploy_current_price > 0:
        ...
        if st.session_state.prepared_orders:
            ...
            with debug_col1:
                st.caption(f"🔌 Broker: {broker_connected} ({'✅' if broker_connected else '❌'})")
            with debug_col2:
                st.caption(f"📦 Orders: {len(st.session_state.prepared_orders)}")
            with debug_col3:
                st.caption(f"🔑 Instance: {broker_instance is not None}")
            ...
            if execute_disabled:
                ...
                if len(st.session_state.prepared_orders) == 0:
                    reasons.append("No orders prepared")
                st.warning(f"⚠️ Execute button disabled: {', '.join(reasons)}")
            ...
            if st.button(button_text, type=button_color, use_container_width=True, disabled=execute_disabled, key="execute_buy_button"):
                # Execute all prepared orders
                print(f"🚀 EXECUTE BUTTON CLICKED! Orders={len(st.session_state.prepared_orders)}, Broker={broker_instance is not None}")
                st.write("🚀 **Button clicked! Processing orders...**")
                executed_count = 0
                failed_count = 0
                order_ids = []
                
                if not broker_instance:
                    st.error("❌ **Critical Error**: Broker instance is None. Cannot execute orders.")
                    st.info(f"Debug: selected_broker={selected_broker_deploy}, broker_connected={broker_connected}")
                    st.stop()
                
                print(f"Processing {len(st.session_state.prepared_orders)} orders...")
                for order in st.session_state.prepared_orders:
                    try:
                        print(f"Processing order: Trench {order.get('trench')}, Symbol {order.get('symbol')}, Qty {order.get('quantity')}")
                        ...
                        print(f"📤 Submitting order to OMS: {order['symbol']} x{order['quantity']}")
                        try:
                            print(f"   Creating order object: {oms_order}")
                            submitted_order = oms.create_order(oms_order, user_id)
                            print(f"✅ Order submitted! Status: {submitted_order.status if hasattr(submitted_order, 'status') else 'UNKNOWN'}")
                            print(f"   Broker Order ID: {submitted_order.broker_order_id if hasattr(submitted_order, 'broker_order_id') else 'NONE'}")
                            print(f"   Rejection Reason: {submitted_order.rejection_reason if hasattr(submitted_order, 'rejection_reason') else 'NONE'}")
                        except Exception as oms_error:
                            failed_count += 1
                            error_details = str(oms_error)
                            print(f"❌ OMS submission error: {error_details}")
                            st.error(f"❌ Trench {order['trench']}: Order submission failed - {error_details}")
                            if 'not supported' in error_details.lower() or 'invalid symbol' in error_details.lower():
                                st.warning(f"💡 Tip: Symbol {order['symbol']} may not be supported by {selected_broker_deploy}. Try a US stock like AAPL, GOOGL, or MSFT.")
                            continue
                        ...
                        if order_status == "REJECTED":
                            ...
                            print(f"✅ Order accepted! ID: {submitted_order.broker_order_id or 'PENDING'}, Status: {order_status}")
                            st.success(f"✅ Trench {order['trench']}: Order submitted successfully! Symbol: {order['symbol']}, Qty: {order['quantity']}, ID: {submitted_order.broker_order_id or 'PENDING'}")
                ...
                # Refresh buying power after order execution
                if executed_count > 0 and broker_instance:
                    try:
                        if selected_broker_deploy == 'Alpaca':
                            account = broker_instance.api.get_account()
                            new_buying_power = float(account.buying_power)
                            st.session_state[f'deploy_buying_power_{selected_broker_deploy}'] = new_buying_power
                            print(f"💰 Buying power refreshed: ${new_buying_power:,.2f}")
                        elif selected_broker_deploy == 'IBKR':
                            account_info = broker_instance.get_account_info()
                            new_buying_power = account_info.get('buying_power', 0.0)
                            st.session_state[f'deploy_buying_power_{selected_broker_deploy}'] = new_buying_power
                            print(f"💰 Buying power refreshed: ${new_buying_power:,.2f}")
                    except Exception as e:
                        print(f"⚠️ Could not refresh buying power: {e}")
